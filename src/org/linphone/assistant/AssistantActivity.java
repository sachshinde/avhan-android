...
public class AssistantActivity extends Activity implements OnClickListener {
		...
		mListener = new LinphoneCoreListenerBase() {
			...
			public void registrationState(LinphoneCore lc,
				...
				if (accountCreated && !newAccount) {
					if (address != null
							&& address.asString().equals(cfg.getIdentity())) {
						if (state == RegistrationState.RegistrationOk) {
							if (LinphoneManager.getLc().getDefaultProxyConfig() != null) {
								System.out
										.println("RegistrationOk..............................................");
	...
	public void onBackPressed() {/*
								 ...
								 * LinphonePreferences
								 * .instance().firstLaunchSuccessful(); if
								 * (getResources
								 * ().getBoolean(R.bool.setup_cancel_move_to_back
								 * )) { moveTaskToBack(true); } else {
								 * setResult(Activity.RESULT_CANCELED);
								 * finish(); } } else if (currentFragment ==
								 ...
								 * WelcomeFragment fragment = new
								 * WelcomeFragment(); changeFragment(fragment);
								 * currentFragment =
								 * AssistantFragmentsEnum.WELCOME;
								 * back.setVisibility(View.INVISIBLE); } else if
								 * (currentFragment ==
								 * AssistantFragmentsEnum.WELCOME) { finish(); }
								 */
	}
	...
	private void logIn(String username, String password, String displayName,
		...
		if (checkconn == false) {
			...
			try {
				...
				URL GenerateKeyUrl = new URL(getString(R.string.app_login_url)+"methodname=generateKey&data="
								+ URLEncoder.encode("{\"appid\":" + ls.appid
										+ "}", "UTF-8"));
				// URL GenerateKeyUrl = new
				// URL("https://demo2.avhan.com:9443/LoginWebService/indexLogin.jsp");
				// System.out.println("GenerateKeyUrl : "+GenerateKeyUrl);
				BufferedReader br = new BufferedReader(new InputStreamReader(
						GenerateKeyUrl.openStream()));
				String strTemp = "";
				...
				strEPassword = obj1.ConvertString(strEncryptPassword
						+ json.get("keyvalue1"));

				URL LoginUrl = new URL(getString(R.string.app_login_url)+"methodname=login&data={\"password\":"
								+ "\""
								+ strEPassword
								+ "\""
								+ ",\"loginid\":"
				...
				if (varResult.equals("1")) {
					...
					String varAcssessionid = json1.get("acssessionid")
							.toString();
					...
					URL SIPServerUrl = new URL(getString(R.string.app_sip_url)+"data="
					...
					BufferedReader br2 = new BufferedReader(
							new InputStreamReader(SIPServerUrl.openStream()));
					...
					JSONObject json2 = (JSONObject) parser.parse(strTemp2
							.substring(15));
					...
					System.out.println("stunservers : "
							+ sipserverjson.get("sipserverip"));
					/* String varStatus = json2.get("status").toString(); */
					String varAcssessionid2 = json2.get("acssessionid")
							.toString();
					String extension = json2.get("extension").toString();
					String pass = json2.get("password").toString();
					System.out
							.println("varAcssessionid2 : " + varAcssessionid2);
					String sipdomain = sipserverjson.get("sipserverip")
							.toString();
					...
					saveCreatedAccount(extension, pass, username, sipdomain,
							transport);
					// saveCreatedAccount("sachin88", "sachin88", "displayName",
					// domain, transport);
					...
					t.scheduleAtFixedRate(new TimerTask() {
						...
						public void run() {
							try {
								...
								String acsessionid = prefs.getString(
										"acsessionid", "No acsessionid found");// "No acsessionid defined"
																				// is
																				// the
																				// default
																				// value.
								// System.out.println("in shared preference :"+acsessionid);
								URL AliveUrl = new URL(getString(R.string.app_sip_url)+"data="
												+ URLEncoder
														.encode("{\"command\":"
																+ ls.aliveCommand
																+ ",\"acssessionid\":"
								...
								BufferedReader br3 = new BufferedReader(
										new InputStreamReader(AliveUrl
												.openStream()));
								...
								JSONObject json3 = (JSONObject) parser
										.parse(strTemp3.substring(15));
								String varStatus = json3.get("status")
										.toString();
								...
								if (!varStatus.equals("3")) {
									...
									Toast.makeText(AssistantActivity.this,
											...
											Toast.LENGTH_SHORT).show();
									try {
										URL LogoutUrl = new URL(getString(R.string.app_sip_url)+"data="
														+ URLEncoder
																.encode("{\"acssessionid\":"
																		+ "\""
																		+ acsessionid
										...
										BufferedReader br5 = new BufferedReader(
												new InputStreamReader(LogoutUrl
														.openStream()));
										// System.out.println("br4 :"+br5.readLine());
										String strTemp5 = "";
										while (null != (strTemp5 = br5
												.readLine())) {
											if (strTemp5.isEmpty()) {
												continue;
											} else {
										...
										JSONObject json5 = (JSONObject) parser
												.parse(strTemp5.substring(15));
										String varStatus5 = json5.get("status")
												.toString();
										quit();
					...
					SharedPreferences prefs = getSharedPreferences(
							MyPREFERENCES, MODE_PRIVATE);
					String acsessionid = prefs.getString("acsessionid",
							"No acsessionid found");// "No name defined" is the
													// default value.
					System.out.println("in shared preference :" + acsessionid);
					try {
						...
						BufferedReader br4 = new BufferedReader(
								new InputStreamReader(LogoutUrl.openStream()));
						...
						// );
						JSONObject json4 = (JSONObject) parser.parse(strTemp4
								.substring(15));
						String varStatus = json4.get("status").toString();
						quit();
						accountCreated = false;
						Intent GooglePlusSignInActivity = new Intent(this,
	...
	public void displayMenu() {
		...
		SharedPreferences prefs = getSharedPreferences(MyPREFERENCES,
				MODE_PRIVATE);
		String acsessionid = prefs.getString("acsessionid",
				"No acsessionid found");// "No acsessionid defined" is the
										// default value.
		...
		if (password == null) {
			...
			try {
				...
				JSONParser parser = new JSONParser();
				JSONObject json3 = (JSONObject) parser.parse(strTemp3
						.substring(15));
				...
				if (!varStatus.equals("3")) {
					...
					try {
						...
						JSONObject json5 = (JSONObject) parser.parse(strTemp5
								.substring(15));
						String varStatus5 = json5.get("status").toString();
						quit();
						logIn(userid, password, userid,
								getString(R.string.default_domain), null, false);
						// System.out.println("Status :"+varStatus5);
						// System.out.println("statusdesc : "+json5.get("statusdescription").toString());
						// System.out.println("revisionno : "+json5.get("revisionno").toString());
		...
		/*
		 * fragment = new WelcomeFragment(); changeFragment(fragment);
	...
	public void saveCreatedAccount(String username, String password,
		...
		if (isMainAccountLinphoneDotOrg) {
			...
			builder.setNoDefault(false).setOutboundProxyEnabled(true);
			if (transport != null) {
